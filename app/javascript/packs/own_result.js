document.addEventListener('turbolinks:load', function() {
  function getLocationAndSortByDistance() {
    navigator.geolocation.getCurrentPosition(function(position) {
      ...
      const url = `/prices/own_prices_result?${query.toString()}`;
  ...
  const sortLink = document.getElementById('favorite-sort-by-distance');
  if (sortLink) {
  ...
  const priceSortByGramLink = document.getElementById('favorite-sort-by-gram');
  if (priceSortByGramLink) {
    priceSortByGramLink.addEventListener('click', function(event) {
      ...
      window.location.href = `/prices/own_prices_result?${query.toString()}`;
    });
  }

  const priceSortByQuantityLink = document.getElementById('favorite-sort-by-quantity');
